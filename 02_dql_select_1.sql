...
select
    menu_name as 메뉴명,
    menu_price as "메뉴 가격", # 쌍또옴표를 이용해서 띄어쓰기된 컬럼 묶기
...
where
    ...
    menu_price not between 10000 and 20000 # 10000~20000
order by
    menu_price desc;
...
where
    ...
    # menu_name like '%쥬스'; # '쥬스'로 끝나는 메뉴 조회
    menu_name like '_____'; # 메뉴명이 5글자인 메뉴 조회

-- =========================================================
# DB에서 NULL == 저장된 데이터가 없다

# 비교 연산 '=' : 비교하는 두 값(data) 이 같으면 Ture

# 카테고리 테이블에서 ref_category_code가 null 인 행 조회
select
    *
from
    tbl_category
where
    # ref_category_code 컬럼 안에 null이란 값이 있으면 Ture
    # -> null은 값(data)가 아님. 컬럼이
    # 컬럼이 비었음을 나타내는 기호
    # ref_category_code = null; # 모든 행 false

    # ref_category_code 컬럼 값이 비어있으면 Ture
...
# 페이징(게시글 페이지 ex)<1,2,3,4,5...>) 처리 시 limit를 사용
